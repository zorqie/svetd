...
use crate::engine::cue_store::CueStore;
use crate::engine::effects::EffectManager;
use crate::engine::fixture::Fixture;
use crate::engine::profile::OflProfile;
use crate::{input, output::artnet::ArtNetSender, param::{Params, FloatParam}, timing::Timer};
...
impl Engine {
    pub fn default() -> Self {
        ...
        if let Some(patch_store) =
            crate::engine::patch::PatchStore::load_from_file("config/patch.json")
        {
            for (id, entry) in patch_store.fixtures {
                let profile_path = format!("profiles/{}.json", entry.profile);
            ...
            log::warn!("Could not load config/patch.json");
        }
        ...
        Self {
            ...
            cue_store: Arc::new(RwLock::new(CueStore::load_from_file("config/cues.json"))),
    ...
    pub fn start(&self) {
        ...
        let a = ArtNetSender::new("2.0.0.1", 0);
        a.start(data_clone);
        ...
        let osc_rx = input::osc::start_osc_thread();
                    ...
                    for cmd in cue.commands {
                        ...
                        match targets {
                            ...
                            crate::engine::target::Target::Groups(groups) => {
                                for g in groups {
                                    let (start, end) = match g.as_str() {
                                        "Top" => (1, 4),
                                        "Bottom" => (5, 8),
                                        _ => (1, 1),
                                    };
                                    for i in start..=end {
                                        let fx_name = format!("F{}", i);
                        ...
                        match cmd {
                            ...
                            crate::engine::command::Command::SetColor { .. } => {
                                let mut em = effect_manager_exec.write().unwrap();
                                for fix in &targeted_fixtures {
                                    if let Some(p) = fix.parameters.get("Red") { em.clear_effects_for(p); }
                                    if let Some(p) = fix.parameters.get("Green") { em.clear_effects_for(p); }
                                    if let Some(p) = fix.parameters.get("Blue") { em.clear_effects_for(p); }
                                    if let Some(p) = fix.parameters.get("White") { em.clear_effects_for(p); }
                                    fix.handle_command(&cmd);
                                }
                            }
                            crate::engine::command::Command::SetPosition { .. } => {
                                let mut em = effect_manager_exec.write().unwrap();
                                for fix in &targeted_fixtures {
                                    if let Some(p) = fix.parameters.get("Pan") { em.clear_effects_for(p); }
                                    if let Some(p) = fix.parameters.get("Tilt") { em.clear_effects_for(p); }
                                    fix.handle_command(&cmd);
                                }
                            }
